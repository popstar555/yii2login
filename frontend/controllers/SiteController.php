...
class SiteController extends Controller
{
        ...
        public function GoogleSuccessCallback($client)
    {
        ...
        $googleAttributes['displayName'] = explode(' ', $googleAttributes['name']);

        $googleAttributes['first_name'] = (!empty($googleAttributes['given_name'])) ?
            $googleAttributes['given_name'] : null;
        $googleAttributes['last_name'] = (!empty($googleAttributes['family_name'])) ?
            $googleAttributes['family_name'] : null;

        $customer = User::findByEmail($googleAttributes['email']);

        if (empty($customer)) {
            $customer               = new User();
            $customer->username   = $googleAttributes['given_name'];
            $customer->email        = $googleAttributes['email'];
        ...
        Yii::$app->session->setFlash('success', 'Google login successfull');
